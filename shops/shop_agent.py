"""
WebOrdo Магазины — Агент печати чеков
=========================================
Печатает чеки кассы магазина на физический принтер. Работает независимо
от других агентов (ресторан, симрейсинг).
...
Файлы:
    shop_agent.py   — этот файл
    shop_config.json — скачайте из личного кабинета → Филиал → Принтер чеков
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "shop_agent.log", encoding="utf-8"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("shop_agent")

CONFIG_PATH = Path(__file__).parent / "shop_config.json"
...
def load_config():
    if not CONFIG_PATH.exists():
        log.error(f"ОШИБКА: shop_config.json не найден: {CONFIG_PATH}")
        log.error("Скачайте shop_config.json из личного кабинета → Филиал → Принтер чеков.")
    ...
    try:
        ...
        log.error(f"ОШИБКА: shop_config.json повреждён: {e}")
        input("Нажмите Enter для выхода...")
        sys.exit(1)
    for key in ("server_url", "token"):
        if not cfg.get(key):
            log.error(f"ОШИБКА: в shop_config.json отсутствует поле '{key}'.")
            input("Нажмите Enter для выхода...")
            sys.exit(1)
...
class ApiClient:
    ...
    def get_jobs(self):
        r = self.session.get(f"{self.base}/api/shop-print/jobs/", timeout=15)
        r.raise_for_status()
    ...
    def ack_job(self, job_id, status, error=""):
        ...
        self.session.post(
            f"{self.base}/api/shop-print/jobs/{job_id}/ack/",
            json=payload, timeout=10,
        )

    def heartbeat(self):
        r = self.session.post(f"{self.base}/api/shop-print/heartbeat/", timeout=10)
        r.raise_for_status()
        return r.json()

    def get_config(self):
        r = self.session.get(f"{self.base}/api/shop-print/config/", timeout=10)
        r.raise_for_status()
        return r.json()
...
def main():
    ...
    log.info("=" * 48)
    log.info("WebOrdo Магазины — Агент печати")
    ...
    try:
        ...
        log.info(f"Филиал: {srv.get('branch')}")
        log.info(f"Принтер (сервер): {printer_name}")
    except Exception as e:
        log.warning(f"Не удалось получить конфиг с сервера: {e}")
        log.warning(f"Используется принтер из shop_config.json: {printer_name}")

    if not printer_name:
        log.error("ОШИБКА: имя принтера не задано! Укажите 'printer' в shop_config.json.")
