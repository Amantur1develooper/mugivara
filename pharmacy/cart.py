...
from .models import PharmacyBranch, BranchDrug

SESSION_KEY = "ph_cart"  # session["ph_cart"] = { "<branch_id>": { "<branch_drug_id>": qty } }
...
def add_to_cart(request, branch_id: int, branch_drug_id: int, qty: int = 1):
    qty = max(1, min(int(qty or 1), 99))
    cart = get_cart(request, branch_id)
    k = str(branch_drug_id)
    cart[k] = int(cart.get(k, 0)) + qty
    _save(request, branch_id, cart)
    return cart

def set_qty(request, branch_id: int, branch_drug_id: int, qty: int):
    cart = get_cart(request, branch_id)
    k = str(branch_drug_id)
    qty = int(qty or 0)
    if qty <= 0:
...
def cart_details(branch: PharmacyBranch, cart: dict):
    ids = []
    for k in cart.keys():
        try:
            ids.append(int(k))
    ...
    for k, qty in cart.items():
        ...
        subtotal += line_total
        qty_total += qty
        rows.append({"bd": bd, "drug": bd.drug, "qty": qty, "line_total": line_total})

    rows.sort(key=lambda r: (r["bd"].sort_order, r["bd"].id))
